...
from ui.widgets import Card, ToggleSwitch, HotkeyRecorder
...
class GeneralTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        body.addWidget(SettingRow("Launch at Windows startup", self.launch_toggle,
                                   "Start VoiceFlow AI automatically when you sign in."))
        ...
        body.addWidget(SettingRow("Show floating AI orb", self.orb_toggle,
                                   "Display a small floating indicator while VoiceFlow AI is active."))
        body.addWidget(_divider())
        ...
        theme_combo.addItems(["Dark (default)", "Light", "Match Windows Theme"])
        theme_combo.setFixedWidth(180)
        theme_combo.currentTextChanged.connect(lambda v: self.setting_changed.emit("theme", v))
        body.addWidget(SettingRow("App theme", theme_combo))
...
class AudioTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        silence_combo = QComboBox()
        silence_combo.addItems(["0.5s", "1.0s", "1.5s", "2.0s"])
        silence_combo.setCurrentText("1.0s")
        silence_combo.setFixedWidth(100)
        silence_combo.currentTextChanged.connect(lambda v: self.setting_changed.emit("silence_timeout", v))
        body.addWidget(SettingRow("Silence timeout before auto-stop", silence_combo))
