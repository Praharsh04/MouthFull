...
class LatencyBar(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self._data = {"capture": 12, "stt": 180, "llm": 340, "inject": 8}
...
class PerformancePage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.avg_label = self._stat_label("Avg Latency", "540 ms")
        self.p95_label = self._stat_label("P95 Latency", "810 ms")
        self.req_label = self._stat_label("Requests (session)", "128")
        ...
        # Demo data generator so the page looks alive; backend will replace this.
        self._demo_timer = QTimer(self)
        self._demo_timer.timeout.connect(self._push_demo_data)
        self._demo_timer.start(900)
        self._cpu_base, self._ram_base, self._gpu_base = 18, 34, 6
    ...
    def _push_demo_data(self):
        self._cpu_base = max(4, min(90, self._cpu_base + random.uniform(-6, 6)))
        self._ram_base = max(10, min(95, self._ram_base + random.uniform(-3, 3)))
        self._gpu_base = max(0, min(80, self._gpu_base + random.uniform(-4, 8)))
        self.update_metric("cpu", self._cpu_base)
        self.update_metric("ram", self._ram_base)
        self.update_metric("gpu", self._gpu_base)
    ...
    def stop_demo_mode(self):
        """Call once a real backend is connected to stop the placeholder timer."""
