...
from environment variables prefixed with ``MOUTHFULL_``.
...
from mouthfull.utils.exceptions import ConfigError

def get_appdata_dir() -> Path:
    """Get the standard Windows AppData path for MouthFull."""
    import os
    appdata = os.environ.get('APPDATA', '')
    if not appdata:
        appdata = str(Path.home() / 'AppData' / 'Roaming')
    
    path = Path(appdata) / 'MouthFullLocal'
...
class LoggingConfig(BaseModel):
    ...
    level: str = "INFO"
    log_file: str | None = "logs/mouthfull.log"
    rotation: str = "10 MB"
    retention: int = 3
