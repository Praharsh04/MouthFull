...
from mouthfull.ui.widgets import Card, StatusBadge, AudioLevelMeter
...
class DashboardPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for label, action_id, icon_id in [
            ("Mute Mic", "mute_mic", "fa5s.microphone-slash"),
            ("Clear History", "clear_history", "fa5s.trash-alt"),
            ("Test Injection", "test_injection", "fa5s.vial"),
            ("Reload Model", "reload_model", "fa5s.sync-alt"),
        ]:
            btn = QPushButton(f"  {label}")
            btn.setIcon(qta.icon(icon_id, color="#a7a8ac"))
    ...
    def _on_toggle_clicked(self):
        ...
        if self._is_active:
            ...
            self.toggle_btn.setIcon(qta.icon("fa5s.pause", color="white"))
            self.toggle_btn.setProperty("variant", "danger")
            self.on_start_clicked.emit()
        else:
            self.toggle_btn.setText(" Start Listening")
            self.toggle_btn.setIcon(qta.icon("fa5s.play", color="white"))
            self.toggle_btn.setProperty("variant", "primary")
    ...
    def add_activity_entry(self, text: str, kind: str = "transcript", timestamp: str = "now"):
        import qtawesome as qta
        icons = {"command": "fa5s.bolt", "transcript": "fa5s.file-alt", "system": "fa5s.cog", "error": "fa5s.times-circle"}
        item = QListWidgetItem(f"  {text}   —   {timestamp}")
        item.setIcon(qta.icon(icons.get(kind, "fa5s.circle"), color="#a7a8ac"))
    ...
    def set_running_silent(self, running: bool):
        ...
        self.toggle_btn.setIcon(qta.icon("fa5s.pause" if running else "fa5s.play", color="white"))
